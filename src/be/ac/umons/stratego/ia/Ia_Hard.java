...
public class Ia_Hard {
    ...
    public void play(PlateauBase plateau){
        ...
        for (int i=9;i>=0;i--){
            for (int j=0;j<9;j++){
                if (plateau.board[i][j] != null && plateau.board[i][j].getThisriverpiece()==null ) {
                    ...
                    Pion ennemy = spiraleAnalyse(thispawn, plateau);
                    if (thispawn.getSquad().equals("Ennemy") && ennemy!=null) {
        ...
        //ici on fera un déplacement aléatoire
    ...
    public void analyseBestChoice(Pion ourpawn , Pion piontoattack, PlateauBase plateau){ // regarde le meilleur déplacement à effectuer , fais rien si aucune bonne solution
